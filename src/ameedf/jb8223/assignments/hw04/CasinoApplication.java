 ...
 * DrugsDealer: a Person who is also a Dealer. Has a list of AddictedPlayers to deal to and a specific amount of drugs to deal (in grams).
 * Each addict will consume 10g each time. When there are no drugs to deal, the dealing action will do nothing.
 * Santa: a Dealer of toys. Has a limited number of toys to deal. When there are no toys to deal, the dealing action will do nothing
 * AddictedPlayer: a player that can consume drugs (Consumer)
 * Casino: a place that all those entities can meet. Check the class for more information
 */
